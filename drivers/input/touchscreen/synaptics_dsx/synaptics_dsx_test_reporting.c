...
#define ORIGIN_THRESHOLD 25
#define NEW_THRESHOLD 30

/*FIH, Hubert, 20160107, add selftest range for each firmware {*/
extern unsigned int get_device_config_id;
#define TP_FW_V29 0xDD000029
#define TP_FW_V33 0xDD000033
#define TP_FW_V34 0xDD000034
#define TP_UPPER_LIMIT_V29 4000
#define TP_LOWER_LIMIT_V29 1800
#define TP_UPPER_LIMIT_V33 2530
#define TP_LOWER_LIMIT_V33 1080
#define TP_UPPER_LIMIT_V34 2600
#define TP_LOWER_LIMIT_V34 1400
int tp_upper_limit = TP_UPPER_LIMIT_V34;
int tp_lower_limit = TP_LOWER_LIMIT_V34;
/*} FIH, Hubert, 20160107, add selftest range for each firmware*/

//extern int synaptics_test_reset_device(struct synaptics_rmi4_data *rmi4_data);
...
#define  CFG_F54_TXCOUNT    14           // Number of Transmitters on device under test
#define  CFG_F54_RXCOUNT    25           // Number of Receivers    on device under test
...
enum test_results TestResult = TEST_INVALID;
...
static ssize_t synaptics_rmi4_f54_test_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static DEVICE_ATTR(test, 0666,synaptics_rmi4_f54_test_show, synaptics_rmi4_f54_test_store);
...
static void remove_sysfs(void)
{
	...
	sysfs_remove_file(&rmi4_data->input_dev->dev.kobj, &dev_attr_test.attr);
	dev_err(rmi4_data->pdev->dev.parent,"%s: remove sysfs test\n",__func__);
	return;
}
...
/* NBQ - sandyyschien - [06-536] - [Touch][FTM] Add more test items to touchpanel selftest */
/*  NBQ - AlbertWu - [NBQ-52] - [Touch] Modify touchpanel selftest command. */
#if 0
static ssize_t synaptics_rmi4_f54_selftest(enum f54_report_types report_type)
{
	...
	int i, k, data_interval=0;
	int F54_RX_TO_RX1_lower_bound = 900;
	int F54_RX_TO_RX1_upper_bound = 1100;
	short temp;
/* NBQ - sandyyschien - [06-559} - [Touch][FTM] Add more test items to touchpanel selftest */
	unsigned short numberOfReportRaws = 0;
/* end NBQ - [06-559] */

	pr_info("[Synaptics] :  %s f54_report_types %d\n", __func__, report_type);

	numberOfColums = f54->rx_assigned;
	numberOfRows = f54->tx_assigned;

	printk("[Synaptics] :  %s numberOfColums=%d numberOfRows=%d \r\n", __func__,numberOfColums,numberOfRows);

	retval = synaptics_rmi4_f54_do_preparation_set(1);
	if (retval)
		goto error_preparation_set;

	retval = synaptics_rmi4_f54_report_type_set(report_type);
	if (retval)
		goto error_report_type_set;

	retval = synaptics_rmi4_f54_get_report_set(1);
	if (retval)
		goto error_get_report_set;
	...
	printk("[Synaptics] :  %s report_size(%d)\n", __func__, report_size);

	if (!report_size)
		goto error_report_size;
	...
	if (f54->report_data) {
		...
		mutex_unlock(&f54->data_mutex);
		goto error_no_report_data;
	}
	switch(report_type){
		case F54_HIGH_RESISTANCE:
			pr_info("F54_HIGH_RESISTANCE:\n");
			if (f54->report_size < 6){
				TestResult = TEST_FAILED;
				pr_info("Wrong report_size %d! Test Fail!\n", f54->report_size);
				break;
			}
			temp = buffer[0] | (buffer[1] << 8);
			ReportData = ((int)temp);
			if (ReportData < 450 ){
				pr_info("Max Rx Offset = %d\n", ReportData);
			} else {
				TestResult = TEST_FAILED;
				pr_info("Max Rx Offset = %d, should be less than 450! Test Fail!\n", ReportData);
			}
			temp = buffer[2] | (buffer[3] << 8);
			ReportData = ((int)temp);
			if (ReportData < 450 ){
				pr_info("Max Tx Offset = %d\n", ReportData);
			} else {
				TestResult = TEST_FAILED;
				pr_info("Max Tx Offset = %d, should be less than 450! Test Fail!\n", ReportData);
			}
			temp = buffer[4] | (buffer[5] << 8);
			ReportData = ((int)temp);
			if (ReportData > -400 ){
				pr_info("Min Pixel = %d\n", ReportData);
			} else {
				TestResult = TEST_FAILED;
				pr_info("Min Pixel = %d, should be greater than -400! Test Fail!\n", ReportData);
			}
			break;
		case F54_TX_TO_TX_SHORT:
			pr_info("F54_TX_TO_TX_SHORT:\n");
			for (i = 0; i < f54->report_size; i++){
				if (buffer[i] > 0){
					TestResult = TEST_FAILED;
					pr_info("Byte%d = %u, have TX_TO_TX_SHORT! Test Fail!\n", i, buffer[i]);
				} else {
					pr_info("Byte%d = %u\n", i, buffer[i]);
				}
			}
			break;
		case F54_RX_TO_RX1:
			pr_info("F54_RX_TO_RX1:\n");
			if (f54->report_size < (2 * numberOfColums * numberOfRows)){
				TestResult = TEST_FAILED;
				pr_info("Wrong report_size %d! Test Fail!\n", f54->report_size);
				break;
			}
			DataArray = (int*) kmalloc(sizeof(int) * numberOfRows, GFP_KERNEL);
			k=0;
			data_interval = 2 * (numberOfColums + 1);
			for (i = 0; i < numberOfRows; i++, k+=data_interval)
			{
				temp = buffer[k] | (buffer[k+1] << 8);
				DataArray[i] = ((int)temp);
				if ((DataArray[i] < F54_RX_TO_RX1_lower_bound) ||
					(DataArray[i] > F54_RX_TO_RX1_upper_bound)){
					TestResult = TEST_FAILED;
					pr_info("%d, Should between %d~%d! Test Fail!\n", DataArray[i],
						F54_RX_TO_RX1_lower_bound, F54_RX_TO_RX1_upper_bound);
				}else{
					pr_info("%d\n",DataArray[i]);
				}
			}
...
/* NBQ - sandyyschien - [06-559] - [Touch][FTM] Add more test items to touchpanel selftest */
		case F54_RX_TO_RX2:
			pr_info("F54_RX_TO_RX2:\n");
			if(numberOfRows > numberOfColums){
				pr_info("numberOfRows > numberOfColums, report_size = %d!\n", f54->report_size);
				break;
			}else if (f54->report_size < (2 * numberOfColums * (numberOfColums - numberOfRows))){
				TestResult = TEST_FAILED;
				pr_info("Wrong report_size %d! Test Fail!\n", f54->report_size);
				break;
			}
			...
			k = (2 * numberOfRows);
			data_interval = 2 * (numberOfColums + 1);
			numberOfReportRaws = numberOfColums - numberOfRows;
			for (i = 0; i < numberOfReportRaws; i++, k+=data_interval)
			{
				temp = buffer[k] | (buffer[k+1] << 8);
				DataArray[i] = ((int)temp);
				if ((DataArray[i] < F54_RX_TO_RX1_lower_bound) ||
...
error_no_report_data:
	kfree(buffer);
error_alloc_buffer:
error_report_size:
error_get_report_set:
error_report_type_set:
error_preparation_set:
	return retval;
}
#endif
/* end  NBQ - AlbertWu - [NBQ-52] */
/* end NBQ - [06-536] */
static ssize_t fullRawCap(void)
{
	...
	int *DataArray, *FullRaeCapUpperLimit , *FullRaeCapLowerLimit;
	char Result;
	unsigned short numberOfColums,numberOfRows;
	unsigned short CknumberOfColums,CknumberOfRows;
	int i, j, k=0;
	short temp;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	int upper_limit = 0, lower_limit = 0, real_value = 0;
	ssize_t ret = 0;
...
/*} FIH, Hubert, 20160107, add selftest range for each firmware*/
	...
	pr_info("[Synaptics] :  %s ENTER new fullrawcap\n", __func__);
...
/*FIH, Hubert, 20160107, add selftest range for each firmware {*/
	...
	if(get_device_config_id == TP_FW_V29)
	{
		pr_debug("choice_the_limit_range_for_V29\n");
		tp_upper_limit = TP_UPPER_LIMIT_V29;
		tp_lower_limit = TP_LOWER_LIMIT_V29;
	}
	else if(get_device_config_id == TP_FW_V33)
	{
		pr_debug("choice_the_limit_range_for_V33\n");
		tp_upper_limit = TP_UPPER_LIMIT_V33;
		tp_lower_limit = TP_LOWER_LIMIT_V33;
	}
	else if(get_device_config_id == TP_FW_V34)
	{
		pr_debug("choice_the_limit_range_for_V34\n");
		tp_upper_limit = TP_UPPER_LIMIT_V34;
		tp_lower_limit = TP_LOWER_LIMIT_V34;
	}
/*} FIH, Hubert, 20160107, add selftest range for each firmware*/

	TestResult = TEST_INVALID;
/*  NBQ - AlbertWu - [NBQ-52] - [Touch] Modify touchpanel selftest command. */
/* NBQ - sandyyschien - [06-536] - [Touch][FTM] Add more test items to touchpanel selftest */
#if 0	
	synaptics_rmi4_f54_selftest(F54_HIGH_RESISTANCE);
	synaptics_rmi4_f54_selftest(F54_TX_TO_TX_SHORT);
	synaptics_rmi4_f54_selftest(F54_RX_TO_RX1);
/* NBQ - sandyyschien - [06-559] - [Touch][FTM] Add more test items to touchpanel selftest */
	synaptics_rmi4_f54_selftest(F54_RX_TO_RX2);
#endif
/* end NBQ - [06-559] */
/* end NBQ - [06-536] */
/* end  NBQ - AlbertWu - [NBQ-52] */
	...
	pr_info("[Synaptics] :  %s numberOfColums=%d numberOfRows=%d \r\n", __func__,numberOfColums,numberOfRows);

    CknumberOfColums=numberOfColums;//-3;
	CknumberOfRows=numberOfRows;//-2;
	item_number = CknumberOfColums*CknumberOfRows;

	if(rmi4_data->hw_if->board_data->upper_bound_array_size != item_number) {
		retval = -1;
		pr_err("[Synaptics]: the upper bound array size %d error \r\n",rmi4_data->hw_if->board_data->upper_bound_array_size);
		goto error_array_size;
	}

	if(rmi4_data->hw_if->board_data->lower_bound_array_size != item_number) {
		retval = -1;
		pr_err("[Synaptics]: the lower bound array size %d error\r\n",rmi4_data->hw_if->board_data->lower_bound_array_size);
		goto error_array_size;
	}
/* NBQ - sandyyschien - [06-41] - [Touch] Update Synaptics touch firmware */
	RESULT_SIZE = 15100;
/* end NBQ - [06-41] */
	...
	if(ResultArray)
		...
		pr_info("[Synaptics] :  : %s alloc ResultArray memory failed\r\n", __func__);
	...
	RawdataFile = filp_open(TP_rawdata_path, O_CREAT | O_RDWR, (S_IRUGO | S_IWUSR) );
	if(IS_ERR(RawdataFile))
		pr_err("[Synaptics]: %s open /data/TPRawdataFile.bin failed = %ld\n", __func__, PTR_ERR(RawdataFile));

	retval = synaptics_rmi4_f54_do_preparation_set(1);
	if (retval)
		goto error_preparation_set;

	retval = synaptics_rmi4_f54_report_type_set(F54_FULL_RAW_CAP_RX_COUPLING_COMP/*F54_TRUE_BASELINE*/);
	if (retval)
		goto error_report_type_set;
	...
	report_size = f54->report_size;
	pr_info("[Synaptics] :  %s report_size(%d)\n", __func__, report_size);
	...
	/*add the three channel of virtual keys*/
	pr_info("[Synaptics] :  %s item_number(%d)\n", __func__, item_number);

	DataArray = (int*) kmalloc(sizeof(int) * numberOfColums * numberOfRows, GFP_KERNEL);
	if(!DataArray) {
		pr_err("[Synaptics]: error allocate DataArray\n");
		goto error_alloc_DataArray;
	}
	k=0;
	...
	FullRaeCapUpperLimit = (int*) kmalloc(sizeof(int) * item_number, GFP_KERNEL);
	if(!FullRaeCapUpperLimit)
		goto error_alloc_upper_bound;

	FullRaeCapLowerLimit = (int*) kmalloc(sizeof(int) * item_number, GFP_KERNEL);
	if(!FullRaeCapLowerLimit)
		goto error_alloc_lower_bound;

	k = 0;
	for(i = 0; i < CknumberOfColums; i++) {
		for (j = 0; j < CknumberOfRows; j++, k+=2) {
			temp = rmi4_data->hw_if->board_data->upper_bound[k+1] | (rmi4_data->hw_if->board_data->upper_bound[k] << 8);
			FullRaeCapUpperLimit[i*numberOfRows+j] = ((int)temp);
		}
	}
	...
	for(i = 0; i < CknumberOfColums; i++) {
		for (j = 0; j < CknumberOfRows; j++, k+=2) {
			temp = rmi4_data->hw_if->board_data->lower_bound[k+1] | (rmi4_data->hw_if->board_data->lower_bound[k] << 8);
			FullRaeCapLowerLimit[i*numberOfRows+j] = ((int)temp);
		}
	}
	// Check against test limits
	for (i = 0; i < CknumberOfColums; i++) {
		pr_info("\n");
	   	for (j = 0; j < CknumberOfRows; j++) {

			/*the three virtual keys locate at (0,25),(1,25),(13,25)*/
			/* NBQ - sandyyschien - [06-41] - [Touch] Update Synaptics touch firmware */
			//if((j != rmi4_data->hw_if->board_data->rx_of_key) || (i == 0) || (i == 1) || (i == 13)){
//FIH, Hubert, 20160107, add selftest range for each firmware {
				//upper_limit = FullRaeCapUpperLimit[i*numberOfRows+j];
				//lower_limit = FullRaeCapLowerLimit[i*numberOfRows+j];
				upper_limit = tp_upper_limit;
				lower_limit = tp_lower_limit;
//} FIH, Hubert, 20160107, add selftest range for each firmware
				...
				if ((DataArray[i*numberOfRows+j] < upper_limit) && (DataArray[i*numberOfRows+j] > lower_limit))
				{
					Result = 'P'; /*Pass*/
				...
				else
				{
					...
					pr_info("[Synaptics]: row = %d , column = %d , upper limit = %d , lower limit = %d , rawdata = %d\n",j ,i ,upper_limit ,lower_limit ,DataArray[i*numberOfRows+j]);
				}
				//pr_info("F@TOUCH %s [%2d][%2d] (%d) %c\n", __func__,i, j, DataArray[i*numberOfRows+j], Result);
			//}
			/* end NBQ - [06-41] */
		}
	}
...
/*} FIH, Hubert, 20151021, BBox for touch, vibrator, led*/
	...
	len = RawdataFile->f_op->write(RawdataFile, ResultArray, RESULT_SIZE, &RawdataFile->f_pos);
	...
	kfree(buffer);
	kfree(DataArray);
	kfree(FullRaeCapUpperLimit);
	kfree(FullRaeCapLowerLimit);
	kfree(ResultArray);
	set_fs(old_fs);

	/*software reset the touch panel*/
/* NBQ - sandyyschien - [06-41] - [Touch] Update Synaptics touch firmware */
	synaptics_test_reset_device(rmi4_data);
/* end NBQ - [06-41] */
	pr_err("[Synaptics] :  %s EXIT\n", __func__);
	return retval;

error_alloc_lower_bound:
	kfree(FullRaeCapUpperLimit);
error_alloc_upper_bound:
	kfree(DataArray);
error_alloc_DataArray:
error_no_report_data:
	kfree(buffer);
error_alloc_buffer:
error_report_size:
...
error_alloc_array:
error_array_size:
	pr_info("[Synaptics] :  %s Faild to read F54_FULL_RAW_CAP_RX_COUPLING_COMP Result retval = %d\n", __func__, retval);
...
static int synaptics_rmi4_f54_set_sysfs(void)
{
	...
	retval = sysfs_create_file(&rmi4_data->input_dev->dev.kobj, &dev_attr_test.attr);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to create sysfs test file\n",
				__func__);
	}
...
{
	synaptics_rmi4_dsx_new_function(&f54_module, true);
...
{
	synaptics_rmi4_dsx_new_function(&f54_module, false);
